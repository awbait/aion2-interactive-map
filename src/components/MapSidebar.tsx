...
const MapSidebar: React.FC<Props> = ({
	...
	const [isClearCompletedModalOpen, setIsClearCompletedModalOpen] =
		useState(false)
					...
					>
						...
						>
							<div className='grid grid-cols-2 gap-2'>
								...
								{maps.length === 0 && (
									<p className='text-xs text-default-500'>No maps loaded.</p>
								)}
							</div>
						</AccordionItem>
							...
							>
								...
								{loadingMarkers && (
									<div className='flex items-center gap-2 text-xs text-default-500 mb-2'>
										<Spinner size='sm' />
										<span>
											{t('common:loadingMarkers', 'Loading markers…')}
										</span>
									</div>
								)}
									...
									{regions.map(region => {
										...
										return (
											...
											>
												<span className='flex items-center gap-1 min-w-0'>
													<span className='truncate text-left'>
														{t(`${regionNs}:${region.name}.name`, region.name)}
													</span>
												</span>
											</Button>
										)
